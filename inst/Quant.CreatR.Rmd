...
weightData <- read.xls (weightPath, sheet = 1, header = TRUE)
...
# output$dataSampleToPrint <- renderText({
#   weightData$Box.pos
# })
# 
# output$dataSampleToPrint2 <- renderText({
#   quantTable()$Identifier.2
# })
...
output$errorPrint <- renderText({
  if(ToDisplay()[[1]][1] != 0){
    if(!is.na(ToDisplay()[[1]][2])){
      paste("Be Careful, weight file and TCD do not match on their identifier.2, check line(s):", ToDisplay()[[1]][2])
    } else {
