...
tkmessageBox(message = "Choose a TCD file", type = "ok")
...
* General conditions:
    ...
    + Peak.Nr..Flash.TCD column accepts only the following values:1, 2, NA or empty cell
* For standards:
...
output$text.Lineerror_identifier.2.Sample <- renderText({ 
  ...
  return(paste("1. There is(are)", rendR, "error(s) in the identifier.2 column at the line(s):", paste(rendR2, sep = " ", collapse = " ")))
     })
...
<br>
2. Number of line (2 lines for sucrose and 3 for the rest):
`r textOutput('length.sampleError')`
`r textOutput('sampleError')`
<br>
3. Missing values (1 values and 1 empty cell for sucrose, 2 values and 1 empty cell for the rest of the standards)
