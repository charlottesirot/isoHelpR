...
weightData <- read.xls(xls = weightPath, sheet = 1, header = TRUE)
...
return(table2.sample.Karina)
 ...
 output$dataSampleToPrint <- renderDT({
   ...
   if(length(input$C.define) > 1 & length(input$N.define) > 1 ) {
     write.csv(table2.sample.Karina(), file = "table2.sample.Karina.csv")
...
output$FinalTable <- renderDT({
  ...
  if(length(input$C.define) > 1 & length(input$N.define) > 1 ) {
    write.csv(table2.sample.Peter(), file = "table2.sample.Peter.csv")
   table2.sample.Peter()
  } else {}
